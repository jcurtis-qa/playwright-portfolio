...
def test_checkout(logged_in_page: Page):
    ...
    expect(cart_page.inventory_items.filter(has_text="Sauce Labs Onesie")).to_have_count(1)
    expect(cart_page.inventory_items.filter(has_text="Sauce Labs Backpack")).to_have_count(1)
    ...
    assert checkout_two_page.item_total_value() == checkout_two_page.summed_item_value(), "Sum of item prices does not equal subtotal presented during checkout."
    assert (checkout_two_page.item_total_value() + checkout_two_page.tax_value()) == checkout_two_page.total_value(), "Sum of subtotal and tax is not equal to total price during checkout."
    ...
    expect(logged_in_page.locator('[data-test="complete-text"]')).to_have_text("Your order has been dispatched, and will arrive just as fast as the pony can get there!")
